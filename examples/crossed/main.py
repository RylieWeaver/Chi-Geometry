...
def main():
    ...
    device = torch.device('cuda' if model_args['use_cuda'] and torch.cuda.is_available() else 'cpu')
    ...
    for epoch in range(1, model_args['epochs'] + 1):
        ...
        train_accuracies = test(model, train_loader, device, num_classes)
        test_accuracies = test(model, test_loader, device, num_classes)
        
        # Format accuracies
        train_acc_str = ", ".join([f"Class {i}: {acc:.2f}" for i, acc in enumerate(train_accuracies)])
        test_acc_str = ", ".join([f"Class {i}: {acc:.2f}" for i, acc in enumerate(test_accuracies)])
        
        print(f'Epoch: {epoch:03d}, Loss: {loss:.4f}, Train Acc: [{train_acc_str}], Test Acc: [{test_acc_str}]')
