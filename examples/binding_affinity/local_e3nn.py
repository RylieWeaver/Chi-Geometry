...
def main():
    ...
    # Load Data
    train_dataset = torch.load(os.path.join(f"{datadir}/train_sample.pt"))
    val_dataset = torch.load(os.path.join(f"{datadir}/val_sample.pt"))
    test_dataset = torch.load(os.path.join(f"{datadir}/test_sample.pt"))
    print(
    ...
    model = WrappedModel(base, model_args["hidden_dim"], model_args["output_dim"]).to(
        device
    )
